...
// functions
template <size_t points, typename T> 
ac_complex<T> Twiddle(int size, int index, int stage, int stream) {
  ...
  if (size <= (1 << (kTwiddleStages * 2 + 2))) {//shift binary 1 to the left by 12 positions=2^12=4096
...
#pragma unroll        //point=4 or 8,so group=0, or 0,1
    for (int k = 1; k < 4; k++) { //k=1,2,3
      ...
      data[k + group * 4] *= Twiddle<points, T>(size, index, stage, stream);
    } //data[1]-2-3,5-6-7
  }
  return data;
}
...
// #pragma unroll //logn is changing so delete unroll
...
// template
int BitReversed(int bits, int x) {
  int y = 0;
// #pragma unroll //bits changing
 ...
 */
template <int logn_max, int logn_row, int logn_col, 
          size_t log_points, typename PipeOut, typename T>
struct Fetch { //reading matrix data from memory and sending the fetched data to FFT engine using a data pipeline
  ac_complex<T> *src;
  // int logn_row;
  // int logn_col; // in fact, logn_row and logn_col are constexpr, can be in template
  int rowORcol;
  
  Fetch(ac_complex<T> *src_, int rowORcol_) : 
        src(src_), rowORcol(rowORcol_) {}
  ...
  void operator()() const {
    constexpr int kN_max = (1 << logn_max);
    constexpr int kPoints = (1 << log_points); //num of points fetched in one operation.

    int logn_now = rowORcol == 0 ? logn_col : logn_row;  
    int kN = (1 << logn_now);
    //use kWorkGroupSize to represent the num of column in this fft iteration
    int kWorkGroupSize = kN; //column_number
    int kIterations = (1 << logn_col) * (1 << logn_row) / kPoints / kWorkGroupSize; //num of iterations required to process the entire matrix.
               // 2 iteration = 16x16 / 8       /16

    for (int i = 0; i < kIterations; i++) {
      // Local memory for storing 8 rows 
      // ac_complex<T> buf[kPoints * kN];
      ac_complex<T> buf[kPoints * kN_max]; //matrix data is fetched row by row and stored in the buf array.
...
//The data is then read into a local buffer(buf) using the computed memory index
        for (int k = 0; k < kPoints; k++) { //where bitwise 2^7-1(111 1111) +k
          buf[(where & ((1 << (logn_now + log_points)) - 1)) + k] = //just fill in every kpoints bits(k control)
              src[where_global + k];
        }
      }

      for (int work_item = 0; work_item < kWorkGroupSize; work_item++) {
        int row = work_item >> (logn_now - log_points);
...
template <int logn_max, int logn_row, int logn_col, 
          size_t log_points, typename PipeIn, typename PipeOut,
...
struct FFT {
  ...
  FFT(int inverse_, int rowORcol_) :
        inverse(inverse_), rowORcol(rowORcol_) {}

  void operator()() const {
    constexpr int kPoints = (1 << log_points);
    constexpr int kN_max = (1 << logn_max);
    int logn = rowORcol == 0 ? logn_col : logn_row;
    int kN = (1 << logn);
    ...
    ac_complex<T> fft_delay_elements[kN_max + kPoints * (logn_max - 2)]; //sliding window for data reordering
    ...
    for (unsigned i = 0; i < (1 << logn_col) * ((1 << logn_row) / kPoints) + kN / kPoints - 1; i++) { // kN/kPoints determines how many iterations are needed to process all points
      std::array<ac_complex<T>, kPoints> data; //process batch

      // Read data from channels (input pipe)
      if (i < (1 << logn_col) * ((1 << logn_row) / kPoints)) {
 ...
 */
template <int logn_max, int logn_row, int logn_col,
          size_t log_points, typename PipeIn, typename T>
struct Transpose {
  ...
  Transpose(ac_complex<T> *dest_, int rowORcol_) : 
      dest(dest_), rowORcol(rowORcol_) {}
  ...
  void operator()() const {
    ...
    int logn_now = rowORcol == 0 ? logn_col : logn_row; 
    int logn_next = rowORcol == 0 ? logn_row : logn_col; 
    int kN = (1 << logn_now);
    int kN_next = (1 << logn_next);
    int kWorkGroupSize = kN;
    int kIterations = (1 << logn_col) * (1 << logn_row) / kPoints / kWorkGroupSize;

    for (int t = 0; t < kIterations; t++) {
      //Data Buffering
      ac_complex<T> buf[kPoints * kN_max];
...
#pragma unroll
      ...
      for (int work_item = 0; work_item < kWorkGroupSize; work_item++) {
        ...
        int revcolt = BitReversed(logn_now, colt); //bit-reversed column index, transpose write operation to ensure the FFT results are placed correctly
        //Calculating Memory Offsets
        int i = (t * kN + work_item) >> logn_now;
        // int where = colt * kN + i * kPoints; //the memory location (where)the transposed data will be written
        int where = colt * kN_next + i * kPoints;
